...
import { Check, Gauge, Pause, Play, RotateCcw, Trash2, Volume2 } from 'lucide-react'
 ...
 */
function PlayerControlBar({ playing, setPlaying, mode, setMode, loop, setLoop, sentenceIndex, setSentenceIndex, items = sentences, currentMs = 0, durationMs = 0, onSeek, rate = 1, onRateCycle, disabled = false, isBilingual = true }: { playing:boolean; setPlaying:(v:boolean)=>void; mode:SubtitleMode; setMode:(v:SubtitleMode)=>void; loop:number; setLoop:(v:number)=>void; sentenceIndex:number; setSentenceIndex:(v:number)=>void; items?: SubtitleSentence[]; currentMs?: number; durationMs?: number; onSeek?: (ms:number)=>void; rate?: number; onRateCycle?: ()=>void; disabled?: boolean; isBilingual?: boolean }) { const modeLabel = !isBilingual ? (mode === 'chinese' ? '关' : '开') : mode === 'bilingual' ? '双语' : mode === 'english' ? '仅英文' : '仅中文'; const toggleMode = () => { if (!isBilingual) { setMode(mode === 'chinese' ? 'english' : 'chinese'); return } setMode(mode === 'bilingual' ? 'english' : mode === 'english' ? 'chinese' : 'bilingual') }; const next = () => setSentenceIndex(Math.min(items.length - 1, sentenceIndex + 1)); const prev = () => setSentenceIndex(Math.max(0, sentenceIndex - 1)); const pct = durationMs > 0 ? Math.min(100, (currentMs / durationMs) * 100) : 0; const loopLabel = loop === Number.POSITIVE_INFINITY ? '∞' : loop > 0 ? `${loop}次` : ''; const seekFromPointer = (e: React.PointerEvent<HTMLDivElement>) => { if (disabled || !onSeek || durationMs <= 0) return; const rect = e.currentTarget.getBoundingClientRect(); const ratio = Math.min(1, Math.max(0, (e.clientX - rect.left) / rect.width)); onSeek(Math.round(ratio * durationMs)) }; return <div className="flex flex-col gap-3 rounded-2xl border bg-card p-4"><div className="flex items-center gap-3"><span className="text-xs text-muted-foreground tabular-nums">{formatDuration(currentMs)}</span><div role="slider" aria-label="播放进度" aria-valuemin={0} aria-valuemax={durationMs} aria-valuenow={currentMs} tabIndex={disabled ? -1 : 0} className="h-2 flex-1 cursor-pointer rounded-full bg-muted" style={{ touchAction: 'none' }} onPointerDown={(e) => { e.currentTarget.setPointerCapture(e.pointerId); seekFromPointer(e) }} onPointerMove={(e) => { if (e.buttons > 0) seekFromPointer(e) }}><div className="h-full rounded-full bg-primary" style={{ width: `${pct}%` }} /></div><span className="text-xs text-muted-foreground tabular-nums">{formatDuration(durationMs)}</span></div><div className="flex flex-wrap items-center justify-center gap-2"><Button variant="outline" size="icon" onClick={prev} disabled={disabled || sentenceIndex === 0} aria-label="上一句">|◀</Button><Button size="icon" className="size-12 rounded-full" onClick={() => setPlaying(!playing)} disabled={disabled} aria-label={playing ? '暂停' : '播放'}>{playing ? <Pause /> : <Play />}</Button><Button variant="outline" size="icon" onClick={next} disabled={disabled || items.length === 0 || sentenceIndex === items.length - 1} aria-label="下一句">▶|</Button><Button variant={loop ? 'secondary' : 'ghost'} onClick={() => setLoop(loop === 0 ? 1 : loop === 1 ? 3 : loop === 3 ? Number.POSITIVE_INFINITY : 0)} disabled={disabled || items.length === 0}><RotateCcw data-icon="inline-start" />循环{loop ? ` ${loopLabel}` : ''}</Button><Button variant="outline" onClick={onRateCycle} disabled={disabled}><Gauge data-icon="inline-start" />倍速 {rate}x</Button><Button variant="outline" onClick={toggleMode}><Volume2 data-icon="inline-start" />字幕：{modeLabel}</Button></div></div> }
 ...
 */
function SubtitleList({ mode, active, onSelect, items = sentences, favoriteIndexes }: { mode: SubtitleMode; active:number; onSelect:(i:number)=>void; items?: SubtitleSentence[]; favoriteIndexes?: Set<number> }) { const containerRef = useRef<HTMLDivElement | null>(null); const itemRefs = useRef<Map<number, HTMLButtonElement>>(new Map()); const manualUntilRef = useRef(0); const programmaticRef = useRef(false); useEffect(() => { if (active < 0) return; const el = itemRefs.current.get(active); const container = containerRef.current; if (!el || !container) return; if (Date.now() < manualUntilRef.current) return; programmaticRef.current = true; el.scrollIntoView({ block: 'nearest' }); window.setTimeout(() => { programmaticRef.current = false }, 300) }, [active]); const markManual = () => { if (!programmaticRef.current) manualUntilRef.current = Date.now() + 5000 }; return <section className="flex min-h-0 flex-1 flex-col rounded-2xl border bg-card"><div className="flex items-center justify-between border-b p-4"><h2 className="font-serif text-xl">字幕</h2><Badge variant="outline">{mode === 'bilingual' ? '英 / 中' : mode === 'english' ? 'English' : '中文'}</Badge></div><div ref={containerRef} onWheel={markManual} onTouchMove={markManual} className="flex-1 overflow-auto p-2">{items.length ? items.map((s, i) => <button key={s.index} ref={(el) => { if (el) itemRefs.current.set(i, el); else itemRefs.current.delete(i) }} onClick={() => onSelect(i)} className={`w-full rounded-xl p-4 text-left transition-colors ${active === i ? 'bg-primary/10 ring-1 ring-primary/30' : 'hover:bg-muted'}`}><div className="mb-2 flex items-center gap-2 text-xs font-medium text-muted-foreground"><span>{active === i ? '▶' : s.index + 1}</span><span>{formatDuration(s.startMs)}</span>{favoriteIndexes?.has(s.index) && <span aria-label="已收藏" className="text-primary">★</span>}</div>{(mode === 'bilingual' || mode === 'english') && <p className="leading-relaxed">{s.textEn}</p>}{(mode === 'bilingual' || mode === 'chinese') && <p className="mt-1 leading-relaxed text-muted-foreground">{s.textZh}</p>}</button>) : <div className="flex min-h-40 items-center justify-center p-6 text-center text-sm text-muted-foreground">该材料暂无字幕</div>}</div></section> }
