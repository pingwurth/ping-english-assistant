...
> 版本：v1.2 · 状态：待评审
> 变更记录：
> - v1.2（2026-08-25）新增 §1.4 页面原型（page-prototype）与正式工程的关系；§6 目录结构补充 page-prototype；§2.7 / §3.3 补充原型相关说明
> - v1.1（2026-08-24）新增「文字转语音（Kokoro-82M）」功能——ADR-7、§2.8 FileSaver、§3.3-⑥⑦、§5.5 链路、§9 里程碑调整
...
> 范围说明：本文档描述的是**最终生产系统**的架构设计（uni-app 三端 + Fastify 后端）。仓库内 `page-prototype/` 目录是基于 Next.js/React 的页面原型，仅用于页面与交互验证，不是最终生产代码，也不遵循本文档的 uni-app/分层约束（详见 §1.4）。
...
### 1.4 页面原型（page-prototype）与正式工程的关系

仓库内 `page-prototype/` 是独立的页面原型工程，**仅用于页面与交互验证，非最终生产代码**；正式产品代码将按本文档 §6 目录结构在 `client/`（uni-app）与 `server/`（Fastify）中实现。两者关系与差异：

| 项 | 页面原型（page-prototype） | 正式工程（本文档设计） |
|----|--------------------------|----------------------|
| 框架 | Next.js 16.3.0（App Router）+ React 19 + TypeScript | uni-app（Vue 3）三端编译 |
| 样式/UI | Tailwind CSS v4（`@tailwindcss/postcss`）+ shadcn/ui（base-nova 风格，基于 `@base-ui/react`）+ lucide-react | uni-ui + 自研组件（见 §2） |
| 路由 | 页面内 react-router-dom v7 客户端路由，集中于 `components/study-app.tsx`；Next.js 侧 `app/page.tsx` 与 `app/[...slug]/page.tsx` 兜底路由均渲染该组件 | uni-app `pages.json`（见 §2.7） |
| 构建 | `next dev` / `next build`（pnpm） | uni-app CLI + Vite |
| 数据 | 内置 mock 数据，无后端、无本地存储 | 后端 API + 端内本地存储（§3、§4.2） |
| 状态 | React 局部 state | Pinia stores（§2.6） |

原型价值与约束：
- 原型已覆盖 P0–P11 全部页面的页面骨架与核心交互流（详见《原型设计》§1.4、§2.1），正式开发时可参考其布局与交互实现，但代码不可直接移植（技术栈不同）。
- 原型中的真实路由与页面清单见《原型设计》§2.1；原型另含一个探索性页面「音频转文字」（P12，路由 `/transcribe`，未纳入 MVP，后端接口契约待定，见 §3.3 说明）。
- 原型的页面/路由设计若与本文档、《原型设计》冲突，以两份设计文档为准，原型需跟进修正。
...
> 原型对照：`page-prototype` 中的客户端路由（react-router-dom v7）与上表语义一一对应：`/`（材料库）、`/import`、`/player/:materialId`、`/training/:materialId`、`/training/puzzle|dictation|read-aloud|shadowing|recitation|report`、`/settings`、`/tts`，另有原型探索页 `/transcribe`（未纳入 MVP）；完整清单见《原型设计》§2.1。
...
> 原型探索说明：`page-prototype` 中的「音频转文字」页（P12，路由 `/transcribe`）探索了音视频转双语字幕的入口（本机 faster-whisper / 云端大模型转写 / 第三方工具外链三种方式），**该功能未纳入 MVP**，上述三种方式对应的后端/本机接口契约均待定；正式纳入后再在本节补充 API 设计（见《原型设计》§4.13）。
...
├── page-prototype/                  # 页面原型（Next.js 16 + React 19，仅用于页面/交互验证，非生产代码，见 §1.4）
│   ├── app/                         # Next.js App Router（page.tsx / [...slug]/page.tsx 均渲染 study-app）
│   ├── components/                  # study-app.tsx（全部页面与客户端路由） + ui/（shadcn/ui 组件）
